...
import { FaUser, FaShoppingBag, FaTicketAlt, FaSearch, FaFilter, FaPlus, FaHistory, FaInfo } from 'react-icons/fa';
...
const Profile = () => {
  ...
  // Determine active tab based on the hash fragment in the URL
  const hash = location.hash.substring(1); // Remove the # character
  const [activeTab, setActiveTab] = useState(
    hash === 'orders' ? 'orders' : 
    hash === 'vouchers' ? 'vouchers' : 'profile'
  );
